# ============================================================
# MCDS Simulation (Line-transect): Covariate-based Detection Functions
# Author: Gajanan Patil and Shashibhushan Mahadik
# Description:
# This script implements a Monte Carlo simulation study under the 
# Multiple Covariate Distance Sampling (MCDS) framework for line transect data.
#
# Perpendicular distance data are generated using a mixture detection function
...
## Half-Normal (HN) parameters
Bo0 <- 1     # Intercept
Bo1 <- 0.5   # Continuous covariate effect (z1)
B21 <- 0.4   # Categorical covariate effect (z2_B)
B22 <- 0.6   # Categorical covariate effect (z2_C)

## Hazard-Rate (HR) parameters
BB0 <- 1.5   # Intercept
BB1 <- 1     # Continuous covariate effect (z1)
BB21 <- 0.8  # Categorical covariate effect (z2_B)
BB22 <- 1    # Categorical covariate effect (z2_C)

AA0 <- 2     # Shape parameter
...
MM <- 0        # Sample size after detection
# Density estimates
D_Hn1    <- 0
D_HR1    <- 0
D_vreht1 <- 0
...
# Model fit indicators (pass/fail)
pvHN   <- 0
pvHR   <- 0
pvVHRT <- 0

# Model selection indicators (ΔAIC + GOF)
AN6 <- 0
AN7 <- 0
AN8 <- 0
...
##z1x is contineous covariate and z2 is cotegricoal of three factor
op=runif(Ns,0,w)
ind1=which(op<=w)
xx1=op[ind1]
...
Ara=300
   ...
   for( j in 1:mx)
  {
     f3x=function(z)
                {					
			Phi1*exp(-(z^2)/(2*(exp(Bo0+Bo1*z1x[j]+B21*z21[j]+B22*z22[j])^2)))+Phi2*(1-exp(-(z/(exp(BB0+BB1*z1x[j]+BB21*z21[j]+BB22*z22[j])))^(-AA0)))	
                  }
       ...
       for(k in 1:m)
         {
           f2o=function(z)
               {
		zz=(1-(tanh(z/(exp(b0+b1*z1[k]+b21*Z21[k]+b22*Z22[k]))))^a)		
 		...
 		f[k]=(1-(tanh(x[k]/(exp(b0+b1*z1[k]+b21*Z21[k]+b22*Z22[k]))))^a)/mu[k]
...
res12 <- maxBFGS(qq, start=c(5,(1/lm),2,.3,.2))
       ...
       for(k in 1:m)
         {
           f2hn=function(z)
               {
            zzhn=exp(-(z^2)/(2*(exp(b0+b1*z1[k]+bh21*Z21[k]+bh22*Z22[k])^2)))			
 		#zz1=ifelse(zz==NaN,1e-300,zz)
             return(zzhn)
               }
            tthn1=integrate(f2hn,lower=0,upper=w,stop.on.error=FALSE)
            muhn[k]=tthn1$value
 		fhn[k]=(exp(-(x[k]^2)/(2*(exp(b0+b1*z1[k]+bh21*Z21[k]+bh22*Z22[k])^2))))/muhn[k]
       ...
       for(k in 1:m)
         {
           f2hr=function(z)
               {
		zzhr=1-exp(-(z/(exp(b0hr+b1hr*z1[k]+bhr21*Z21[k]+bhr22*Z22[k])))^(-ahr))		
             return(zzhr)
               }
            ttHr=integrate(f2hr,lower=0,upper=w,stop.on.error=FALSE)
            muhr[k]=ttHr$value
 		fhr[k]=(1-exp(-(x[k]/(exp(b0hr+b1hr*z1[k]+bhr21*Z21[k]+bhr22*Z22[k])))^(-ahr)))/muhr[k]
...
resHr12 <- maxBFGS(qqhr, start=c(3,(1/lm),.2,2,2))
...
#print(summary(res))
  ...
  muu=0
   	for( j in 1:m)
  	{
     	f3=function(z)
               {	
			(1-(tanh(z/(exp(B0+B1*z1[j]+BB21*Z21[j]+BB22*Z22[j]))))^AA)						  
                  }
...
##########HN
   ...
   for( j in 1:m)
  {
     f3hn=function(z)
                {					
			exp(-(z^2)/(2*(exp(B0Hn+B1Hn*z1[j]+B21Hn*Z21[j]+B22Hn*Z22[j])^2)))	  
                  }
 	...
 	piHn=MUhn/w
	D_Hn=1/(mean(piHn))
      D_Hn18=(m*mean(f0Hn))/(2*L)
	piHn12=mean(piHn)
#########HR

  	B0Hr=resHr$estimate[1]
  	B1Hr=resHr$estimate[2]
  	AAHr=resHr$estimate[3]
   ...
   for( j in 1:m)
  {
     f3HR=function(z)
               {	
			1-exp(-(z/(exp(B0Hr+B1Hr*z1[j]+B21Hr*Z21[j]+B22Hr*Z22[j])))^(-AAHr))						  
                  }
...
MUu5=0
  for( e in 1:m)
  {
     fx3=function(z)
               {					
		     exp(-(z^2)/(2*(exp(B0Hn+B1Hn*z1[e]+B21Hn*Z21[e]+B22Hn*Z22[e])^2)))	  		
                  }
 ...
 for(h in 1:m)
 {
   fk1=function(z25)
                 {
		     exp(-(z25^2)/(2*(exp(B0Hn+B1Hn*z1[h]+B21Hn*Z21[h]+B22Hn*Z22[h])^2)))	  		
                  }
...
n=m
 MUu22=0
  for( e in 1:m)
  {
     fx3=function(z)
               {					
		     exp(-(z^2)/(2*(exp(B0Hn+B1Hn*Zz11[e]+B21Hn*zz21[e]+B22Hn*zz22[e])^2)))	  		
                  }
 ...
 for(h in 1:m)
 {
   fk13=function(z22)
                 {
		     exp(-(z22^2)/(2*(exp(B0Hn+B1Hn*Zz11[h]+B21Hn*zz21[h]+B22Hn*zz22[h])^2)))	  		
                  }
...
MUu7=0
  for( e in 1:m)
  {
     fx37=function(z)
               {					
			(1-exp(-(z/(exp(B0Hr+B1Hr*z1[e]+B21Hr*Z21[e]+B22Hr*Z22[e])))^(-AAHr)))						  
                  }
 ...
 for(h in 1:m)
 {
   fk17=function(z27)
                 {
			(1-exp(-(z27/(exp(B0Hr+B1Hr*z1[h]+B21Hr*Z21[h]+B22Hr*Z22[h])))^(-AAHr)))						  
                  }
...
n=m
 MUu27=0
  for( e in 1:m)
  {
     fx7=function(z)
               {		
			(1-exp(-(z/(exp(B0Hr+B1Hr*Z17[e]+B21Hr*Z18[e]+B22Hr*Z19[e])))^(-AAHr)))						  
                  }
 ...
 for(h in 1:m)
 {
   fk13=function(z27)
                 {
			(1-exp(-(z27/(exp(B0Hr+B1Hr*Z17[h]+B21Hr*Z18[h]+B22Hr*Z19[h])))^(-AAHr)))						  
                  }
...
######VREHT CvM######
 MUu2=0
  for( e in 1:m)
  {
     fx3=function(z)
               {					
			(1-(tanh(z/(exp(B0+B1*z1[e]+BB21*Z21[e]+BB22*Z22[e]))))^AA)						  
                  }
 ...
 for(h in 1:m)
 {
   fk11=function(z21)
                 {
			(1-(tanh(z21/(exp(B0+B1*z1[h]+BB21*Z21[h]+BB22*Z22[h]))))^AA)						  
                  }
...
######VREHT CvM######
 MUu4=0
  for( e in 1:m)
  {
     fx3=function(z)
               {					
			(1-(tanh(z/(exp(B0+B1*Zz13[e]+BB21*Zz14[e]+BB22*Zz15[e]))))^AA)						  
                  }
 ...
 for(h in 1:m)
 {
   fk14=function(z24)
                 {
			(1-(tanh(z24/(exp(B0+B1*Zz13[h]+BB21*Zz14[h]+BB22*Zz15[h]))))^AA)						  
                  }
...
LLLHr=resHr$maximum
	...
	AIC_TN1[J]=(-2*LLLLTn)+4
	AIC_Hn1[J]=(-2*LLLLhn)+2
	AIC_Hr1[J]=(-2*LLLHr)+4
